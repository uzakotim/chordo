...
function chordToMidiNotes(chord: string): number[] {
  ...
  // Forces the base root note to always live inside the Middle Octave (MIDI 60)
...
function App() {
  ...
  // Track currently held down chord
  ...
  // Generates 4 octaves of piano keys (Octaves 0, 1, 2, 3)
  ...
  // Creates a strict set of absolute MIDI note values currently generated by the held chord
  const activeMidiNotes = useMemo(() => {
    ...
    return new Set(
      notes.map((note) => {
        // Keeps notes inside Octave 1 / 2 (Middle registers) to ensure readability
  ...
  // Triggered instantly when button is pressed down
  ...
  // Triggered when button is released or cursor leaves it
          ...
          <div key={rowIndex} className="chord-row">
            {row.map((chord, colIndex) => (
              <button
                ...
                disabled={!chord}
                // Pointer event handlers enable responsive hold-and-release functionality
